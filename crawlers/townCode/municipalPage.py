...
def Crawler(nth, election_name, electionType, target, target_kor):
	if 1 <= nth <= 3:
		crawler = Municipal_townCodeCrawler_GuOld(int(nth), election_name, electionType, target, target_kor)
	elif 4 <= nth <= 6:
		crawler = Municipal_townCodeCrawler_Old(int(nth), election_name, electionType, target, target_kor)
	elif nth == 7:
		raise InvalidCrawlerError('townCode', nth, election_name, electionType, target, target_kor)
		#"최근선거"로 들어갈 때의 code: crawler = Municipal_townCodeCrawler_Recent(int(nth), election_name, electionType, target, target_kor)
	else:
		raise InvalidCrawlerError('townCode', nth, election_name, electionType, target, target_kor)
	return crawler
