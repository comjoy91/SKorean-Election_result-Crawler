...
class BaseCrawler_PC(object):
	...
	def parse(self, url, params, target, city_code=None, city_name=None):
		...
		if target=='assembly':
			_townType = '국회의원 지역구 목록'
		elif target=='local_eduParliament':
			_townType = '교육의원(광역자치의회) 지역구 목록'
		else:
			raise InvalidCrawlerError(target, 'townCode', nth)
...
class JSONCrawler_PC(BaseCrawler_PC):
	...
	def crawl(self):
		...
		jobs = []
		target = self.target
		townType = "constituency_in_province" #지역구(선거구) 단위 in 광역자치단체

		# 기초자치단체와 선거구 데이터 크롤링의 기본과정.
