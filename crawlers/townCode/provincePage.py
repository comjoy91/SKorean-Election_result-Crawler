...
def Crawler(nth, election_name, electionType, target, target_kor):
	...
	if target == 'president':
		...
		if 1 <= nth <= 3:
			...
			raise InvalidCrawlerError('townCode', nth, election_name, electionType, target, target_kor)
			#"최근선거"로 들어갈 때의 code: crawler = Province_townCodeCrawler_Recent(int(nth), election_name, electionType, target, target_kor)
		else:
			raise InvalidCrawlerError('townCode', nth, election_name, electionType, target, target_kor)
		...
		raise InvalidCrawlerError('townCode', nth, election_name, target, target_kor)
...
class Province_townCodeCrawler_Old(JSONCrawler_province):
	...
	def __init__(self, nth, _election_name, _election_type, _target, _target_kor):
		...
		if _target=='assembly' and nth==17:
			self.urlPath_city_codes = 'http://info.nec.go.kr/bizcommon/selectbox/selectbox_cityCodeBySgJson_GuOld.json'
		else:
