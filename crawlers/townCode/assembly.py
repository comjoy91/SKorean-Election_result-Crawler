...
class LocalDivision_CodeCrawler_Old(JSONCrawler_province):
	...
	def __init__(self, nth, _election_name, _target):
		...
		self.urlParam_city_codes = dict(electionId='0000000000', electionCode=_election_name, subElectionCode=2)
...
class Constituency_CodeCrawler_GuOld(JSONCrawler_province):
	...
	def __init__(self, nth, _election_name, _target):
		...
		self.urlParam_sgg_list = dict(electionId='0000000000', electionName=_election_name, electionCode=2)
...
class Constituency_CodeCrawler_Recent(JSONCrawler_province):
	...
	def __init__(self, nth, _election_name, _target):
		...
		self.urlPath_city_codes = 'http://info.nec.go.kr/bizcommon/selectbox/selectbox_cityCodeBySgJson.json'
		self.urlParam_city_codes = dict(electionCode=2, electionId=_election_name)

		self.urlPath_sgg_list = 'http://info.nec.go.kr/bizcommon/selectbox/selectbox_getSggCityCodeJson.json'
		self.urlParam_sgg_list = dict(electionCode=2, electionId=_election_name)
