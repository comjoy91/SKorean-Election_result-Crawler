...
class LocalDivision_CodeCrawler_GuOld(JSONCrawler_province):
	...
	def __init__(self, nth, _election_name):
		...
		if nth == 17: # n:17 - 이것만 지역구 크롤링을 GuOld가 아니라 그냥 Old에서 진행.
			self.next_crawler = Constituency_CodeCrawler_Old(_election_name)
		else:
			self.next_crawler = Constituency_CodeCrawler_GuOld(_election_name)
...
class LocalDivision_CodeCrawler_Recent(JSONCrawler_province):
	...
	def __init__(self, nth, _election_name):
		self.nth = nth
		self.next_crawler = Constituency_CodeCrawler_Recent(_election_name)
		self.next_crawler.nth = nth
...
class Constituency_CodeCrawler_GuOld(JSONCrawler_province):
	...
	urlPath_sgg_list = 'http://info.nec.go.kr/bizcommon/selectbox/selectbox_getSggCityCodeJson_GuOld.json'
	urlParam_sgg_list = dict(electionId='0000000000', electionCode=2)
...
class Constituency_CodeCrawler_Recent(JSONCrawler_province):
	...
	urlParam_sgg_list = dict(electionCode=2)
