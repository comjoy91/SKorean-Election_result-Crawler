...
def Crawler(nth, election_name, localType_int, target):
	if 1 <= nth <= 3:
		...
		crawler = Constituency_CodeCrawler_Old(int(nth), election_name, localType_int, target)
	elif nth == 7:
...
class Constituency_CodeCrawler_GuOld(MultiCityCrawler_municipal):
	...
	def __init__(self, nth, _election_name, _localType_int, _target):
		...
		if _target == 'local_provincal_parliament' and nth >= 3:
			self.next_crawler = LocalDivision_CodeCrawler_GuOld(nth, _election_name, _localType_int, _target)
...
class Constituency_CodeCrawler_Old(MultiCityCrawler_municipal):
	...
	def __init__(self, nth, _election_name, _localType_int, _target):
		...
		self.next_crawler = LocalDivision_CodeCrawler_Old(nth, _election_name, _localType_int, _target)
...
# TODO: 이것은 선거 전에 반드시 수정해야 함.
		...
		self.urlParam_town_list = dict(electionId=_election_name,\
									...
									electionCode=_localType_int, searchType=3)
		...
		self.next_crawler = LocalDivision_CodeCrawler_Recent(nth, _election_name, _localType_int, _target)
